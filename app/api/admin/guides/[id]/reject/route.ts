...
import { buildGuideRejectionEmail } from "@/lib/emails/guide-application"
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (updateError || !updated) {
      console.error("Reject guide error:", updateError)
    ...
    const email = buildGuideRejectionEmail(guide)
    ...
    if (emailError) {
      console.error("Resend guide rejection email error:", emailError)
      return NextResponse.json({ success: true, guide, emailSkipped: true })
    }

    return NextResponse.json({ success: true, guide })
  } catch (error) {
    console.error("POST /api/admin/guides/[id]/reject error:", error)
